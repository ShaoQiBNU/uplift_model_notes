...
$$ \lambda_B=\max \begin{cases} 0, \lambda_B-\alpha{(B-\sum_{i=1}^M\sum_{j=1}^Nc_{j}x_{ij})} \end{cases} $$

$$ \lambda_C=\max \begin{cases} 0, \lambda_C-\alpha{(\sum_{i=1}^M\sum_{j=1}^N(v_{ij}-v_{i0})t_{j}x_{ij}-C)} \end{cases} $$
